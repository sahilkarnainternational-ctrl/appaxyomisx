...
const scrollToBottom = (behavior: ScrollBehavior = 'smooth') => {\
  ...
  const handleSend = async (manualInput?: string) => {
    ...
    try {
      ...
      // MOCKED API RESPONSE
      const fullContent = `### Welcome to Astra! 🚀

I am a simulated AI assistant. My responses are currently pre-configured. To enable full conversational capabilities, a connection to a backend language model service (like Google\'s Gemini API) is required.

**This demo showcases the following frontend features:**
*   **Real-time UI:** The chat interface you are using right now.
*   **Speech Recognition:** You can use the microphone icon to speak.
*   **Text-to-Speech:** I can read messages aloud (if enabled).
*   **Markdown & Charting:** I can render formatted text, code, and even Mermaid diagrams. For example:
    \`\`\`mermaid
    graph TD
...
This interactive shell is ready to be connected to a powerful AI backend.`;
      ...
      if (isConversationMode || isVoiceOutputEnabled) {
        generateVoice(fullContent);
      }

      markStreamingComplete(assistantMessage.id, fullContent);
