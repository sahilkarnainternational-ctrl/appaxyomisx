...
const DEFAULT_CURRENCY = { code: 'USD', symbol: '$', country: 'United States', flag: '🇺🇸' };
...
function detectFromTimezone() {
    try {
        ...
        if (tz && CURRENCY_BY_TIMEZONE[tz])
            return CURRENCY_BY_TIMEZONE[tz];
        if (tz?.startsWith('Australia/'))
            return { code: 'AUD', symbol: 'A$', country: 'Australia', flag: '🇦🇺' };
...
export function formatPrice(amount, info) {
...
export function useCurrency() {
    const [info, setInfo] = useState(() => detectFromTimezone());
    ...
    useEffect(() => {
        ...
        fetch('https://ipapi.co/json/', { signal: controller.signal })
            ...
            if (resolved) {
                setInfo(resolved);
                try {
                    window.localStorage.setItem('axyomis_geo_currency', JSON.stringify(resolved));
                }
                catch { /* ignore */ }
            }
        })
    ...
    return {
        currency: info,
        pricing,
        format: (amount) => formatPrice(amount, info),
        setCurrency: setInfo,
    };
}
