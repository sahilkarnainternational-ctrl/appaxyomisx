...
export class AxyomisEngine {
    ...
    domCache = { ring: null, card: null, dot: null };
    stopSparkles = false;
    initEventListeners() {
    ...
    renderAnimations() {
        if (document.visibilityState === 'visible') {
            if (!this.domCache.dot)
                this.domCache.dot = document.getElementById('cursor-dot');
            ...
            if (!this.domCache.ring)
                this.domCache.ring = document.getElementById('cursor-ring');
            ...
            if (!this.domCache.card)
                this.domCache.card = document.getElementById('card');
    ...
    async getFallbackImage(title) {
        try {
            ...
            let validImages = pages[pageId].images.filter((img) => {
    ...
    loadMore(cat) {
        ...
        let context = '';
        if (cat === 'study') {
            list = DATA_SETS[this.state.currentStudyTopic];
            context = this.state.currentStudyTopic;
        }
        else if (cat === 'kids') {
            list = DATA_SETS[this.state.currentKidsTopic];
        ...
        `;
            ...
            this.fetchWiki(topic, context).then(data => {
                if (data) {
                    const p = card.querySelector('.st-card-extract');
                    if (p)
                        p.innerText = data.extract ? data.extract.substring(0, 80) + '...' : p.innerText;
                    const imgLayer = card.querySelector('.st-image-layer');
                    if (imgLayer && data.imgSrc) {
                        imgLayer.innerHTML = `<img src="${data.imgSrc}" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-1000 brightness-[1.25] contrast-[1.1] saturate-[1.15]" alt="${topic}" />
    ...
    `;
        ...
        if (data) {
                        ...
                        related.forEach((topic, idx) => {
                            this.fetchWiki(topic, context).then(data => {
                                if (data && data.imgSrc) {
                                    ...
                                    if (imgTarget) {
                                        imgTarget.innerHTML = `<img src="${data.imgSrc}" class="w-full h-full object-cover transition-transform duration-1000 group-hover:scale-110 brightness-[1.3] contrast-[1.1] saturate-[1.2]" alt="${topic}" />
            ...
            if (data.imgSrc) {
                imgWrapper.innerHTML = `<img src="${data.imgSrc}" class="max-w-full max-h-[450px] object-contain rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] border border-white/5 brightness-[1.15] contrast-[1.1] saturate-[1.1]" alt="${q}" />
          <div class="mt-4 text-[9px] uppercase tracking-widest text-[#64748b] font-bold text-center">Spectral Signature Verification: COMPLETE</div>`;
            ...
            if (window.MathJax?.typesetPromise)
                window.MathJax.typesetPromise([extract]);
        }
    }
    ...
    getRelatedTopics(topic, context = '') {
        let list = [];
        if (context && DATA_SETS[context]) {
            list = DATA_SETS[context];
        }
        else {
            const subjects = Object.keys(DATA_SETS);
            for (const s of subjects) {
                if (DATA_SETS[s].includes(topic)) {
                    list = DATA_SETS[s];
                    break;
                }
            }
        ...
        if (list.length === 0)
            list = Object.values(DATA_SETS).flat();
        return list
            .filter(t => t !== topic)
            .sort(() => 0.5 - Math.random())
            .slice(0, 5);
    }
    async loadCategorizedVideos(mainTopic) {
        const videoContainer = document.getElementById('rm-video-container');
        const videoGrid = document.getElementById('rm-video-grid');
        if (!videoContainer || !videoGrid)
            return;
    ...
    playVideoWithQueue(id, title, description, queueIds) {
